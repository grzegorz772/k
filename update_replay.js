...
import { Play, Pause, GripHorizontal, Settings, Skull, Sparkles } from 'lucide-react';
  ...
  return (
    <div className="flex flex-col h-full bg-[#0b0c10] text-[#c5c6c7] p-2 md:p-4 gap-2 md:gap-4 font-sans">
      ...
      <div className="flex-1 flex flex-col xl:flex-row gap-4 min-h-0 overflow-hidden">
        ...
        <div className="xl:w-[450px] shrink-0 flex flex-col gap-4 overflow-y-auto min-h-[300px]">
            <div className="bg-[#111] border border-[#1f2833] rounded-xl p-4 flex flex-col gap-4">
                ...
                <div className="flex flex-col gap-3 text-[11px] text-gray-300">
                    ...
                    <div className="font-bold text-gray-400 uppercase text-[9px] tracking-wider">Champion Gold Lines</div>
                    <div className="grid grid-cols-2 gap-2">
                        {participantStats.map((p) => {
                            const isPlayer = matchDetails.playerPuuid ? participantsInfo[p.id]?.puuid === matchDetails.playerPuuid : false;
                            
                            return (
                                <label key={p.id} className="flex items-center gap-2 cursor-pointer hover:text-white overflow-hidden">
                                    <input type="checkbox" checked={chartConfig.champs[p.id] || false} onChange={e => setChartConfig({...chartConfig, champs: {...chartConfig.champs, [p.id]: e.target.checked}})} className="accent-[#66fcf1]" />
                                    <div className="relative">
                                      <img src={\`https://ddragon.leagueoflegends.com/cdn/\${dDragon.latest}/img/champion/\${p.champion}.png\`} className={\`w-5 h-5 rounded \${isPlayer ? 'border-2 border-emerald-500' : 'border border-gray-700'}\`} />
                                      {isPlayer && <div className="absolute -top-1 -right-1 w-2 h-2 bg-emerald-500 rounded-full"></div>}
                                    </div>
            ...
            <div className="bg-[#111] border border-[#1f2833] rounded-xl p-4 min-h-[250px] flex-1 flex flex-col relative">
                <ResponsiveContainer width="100%" height="100%">
                    <LineChart data={chartData} onClick={(data) => {
                        ...
                        {Object.values(participantsInfo).map((p: any) => {
                            if (!chartConfig.champs[p.participantId]) return null;
                            const isPlayer = matchDetails.playerPuuid ? p.puuid === matchDetails.playerPuuid : false;
                            const idx = (p.participantId - 1) % 5;
                            const color = getChampColor(p.teamId, idx, isPlayer);
                            
                            return (
                                <Line key={p.participantId} type="monotone" dataKey={\`champ_\${p.participantId}\`} name={\`\${p.championInternal || p.championName} Gold (k)\`} stroke={color} dot={false} strokeWidth={2} isAnimationActive={false} />
                            );
                        })}
...
console.log("Updated ReplayViewer.tsx");
